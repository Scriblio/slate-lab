...
Take the **minimum margin over the trajectory** and calibrate the threshold on a
**held-out** draw of in-distribution traffic (the 1% quantile — never fitted on
the evaluation set, never on OOD):

| | clean cues | noisy reads (σ=0.75) |
|---|---|---|
| in-distribution answered | 99.8% ±0.3% | 98.8% ±1.0% |
| unseen enum value answered | 0.0% | 2.7% ±1.2% |
| unknown skill answered | 0.0% | 0.0% |
| far-OOD answered | 0.0% | 0.0% |

At an 85/15 familiar/unfamiliar traffic mix: escalation **15.9% ±0.9%**,
accuracy on answered **99.84% ±0.07%**.

`core.Slate` gained an optional `margin_floor` gate for this. It defaults to
`None`, so every earlier result in this repo stands unchanged.
...
| Slate path latency | median **1.45 ms**, p99 3.40 ms |
| throughput | 611 decisions/sec, one CPU core, no GPU |
| end-to-end latency | 166 ms median vs 1,101 ms all-model |
...
|---|---|---|---|---|---|---|---|---|
| deterministic code | 100.0% | 100.0% | 0.500 | 0.0% | **1.2** | 0 | yes | human |
| rules engine | 100.0% | 100.0% | 0.500 | 0.0% | 2.2 | 0 | yes | human |
| dict (memoised) | 100.0% | 100.0% | **1.000** | **100.0%** | **1.0** | 7,776 | no | labels |
| vector kNN | 84.2% | 58.0% | 0.703 | 1.1% | 9.7 | 1,000 | no | labels |
| **trained classifier** (n=1000) | 99.8% | 99.4% | 0.709 | **16.5%** | 54.8 | 1,000 | no | labels |
| **Slate** (compiled program) | 100.0% | 100.0% | **1.000** | **100.0%** | 1,742 | **0** | yes | model+verifier |
| direct claude-haiku-4-5 | 87.5% | – | – | – | 1,134,290 | 0 | no | prompt |
| direct claude-opus-4-8 | 100.0% | – | – | – | 1,783,422 | 0 | no | prompt |
| direct llama3.2:1b | 25.0% | – | – | – | 2,467,662 | 0 | no | prompt |
...
* **A small local *language* model is not a rival at all here** — llama3.2:1b
  scores 25.0%, below the 33.0% majority-class baseline, and could not author a
...
1. **The escalation result is the load-bearing one, and it needed fixing before
   it was safe.** The shipped familiarity flag would have answered 100% of
   unfamiliar traffic. A calibrated min-margin gate over the trajectory catches
   all of it at no false-alarm cost. That is the difference between a demo and
   something deployable, and it was one measurement away from being missed.
